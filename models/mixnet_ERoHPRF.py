...
class MDConv(nn.Module):
    def __init__(self, channels, kernel_size, stride):
        ...
        kernel_dict = {3: [3], 5: [3, 5], 7: [3, 5, 7], 9: [3, 5, 7, 9], 11: [3, 5, 7, 9, 11],
                       13: [3, 5, 7, 9, 11, 13], 15: [3, 5, 7, 9, 11, 13, 15]}

        self.mixed_depthwise_conv = nn.ModuleList()
        for i in range(self.num_groups):
            self.mixed_depthwise_conv.append(MultiConvBlock(
                self.split_channels[i],
                self.split_channels[i],
                kernel_dict[kernel_size[i]],
                stride=stride,
            ))
        # for i in range(self.num_groups):
    ...
    def merge_kernels(self):
        for layer in self.mixed_depthwise_conv:
            if isinstance(layer, MultiConvBlock):
                layer.mergeMulKernels()
...
class MixNetBlock(nn.Module):
    ...
    def merge_kernels(self):
        for co in self.conv:
            if isinstance(co, nn.Sequential):
                for layer in co:
                    if isinstance(layer, MDConv):
                        layer.merge_kernels()
...
def mixnet(num_classes=3):
    return MixNet(num_classes=num_classes)


if __name__ == '__main__':
    temp = torch.randn((2, 3, 224, 224))
    net = mixnet(num_classes=3)
    net.eval()
    output = net(temp)
    print(output)
    print("---------------")
